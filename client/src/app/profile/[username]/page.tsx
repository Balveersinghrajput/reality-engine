...
import {
  ArrowLeft, Brain, Calendar, CheckCircle, Flame,
  Globe, MessageSquare, Trophy, UserCheck, UserPlus,
  Users, UserX, Zap,
} from "lucide-react";
...
export default function PublicProfilePage() {
  ...
  const acceptRequest = useMutation({
    mutationFn: async () => {
      const res = await api.post(`/connections/accept/${connectionStatus?.connectionId}`);
      return res.data;
    },
    onSuccess: () => {
  ...
  // ✅ FIX 2: use 'profile' consistently (was 'profileUser' which didn't exist)
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#030303", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <button onClick={() => router.back()} style={{ marginTop: "16px", color: "#3b82f6", background: "none", border: "none", cursor: "pointer", fontSize: "13px" }}>
            ← Go back
          </button>
        </div>
      </div>
  ...
  return (
    <>
      ...
      <div className="pp-root">
        ...
        <div className="pp-content" style={{ opacity: mounted ? 1 : 0, transform: mounted ? "translateY(0)" : "translateY(16px)", transition: "opacity 0.5s ease, transform 0.5s ease" }}>
          ...
          <div className="pp-hero">
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: "24px", flexWrap: "wrap" }}>
              ...
              <div className="pp-avatar" style={{ background: `${tier.color}15`, border: `2px solid ${tier.color}40`, boxShadow: `0 0 32px ${tier.glow}`, color: tier.color }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                  ...
                  <button
                    ...
                    disabled={sendRequest.isPending || acceptRequest.isPending || status === "pending_sent"}
                    style={{ background: connBtn.bg, color: connBtn.color, border: `1px solid ${connBtn.border}`, opacity: sendRequest.isPending || acceptRequest.isPending ? 0.7 : 1 }}
                    onMouseEnter={e => ((e.currentTarget as HTMLElement).style.opacity = "0.8")}
                    onMouseLeave={e => ((e.currentTarget as HTMLElement).style.opacity = "1")}
                  >
                    {connBtn.icon}
                    {sendRequest.isPending || acceptRequest.isPending ? "Loading..." : connBtn.label}
                  </button>

                  {/* ✅ FIX 2: changed profileUser.id → profile.id */}
                  {status === "accepted" && (
                    <button
                      onClick={() => router.push(`/chat/${profile.id}`)}
                      style={{ display: "flex", alignItems: "center", gap: "8px", background: "rgba(59,130,246,0.08)", border: "1px solid rgba(59,130,246,0.25)", color: "#3b82f6", borderRadius: "12px", padding: "10px 20px", fontSize: "13px", fontWeight: 700, cursor: "pointer" }}
                    >
              ...
              <div style={{ textAlign: "center", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "16px", padding: "20px 24px", flexShrink: 0 }}>
                <div style={{ fontSize: "36px", fontWeight: 900, color: "#fff", lineHeight: 1 }}>{profile.realityScore || 0}</div>
                <div style={{ fontSize: "10px", color: "#444", marginTop: "6px", fontFamily: "JetBrains Mono, monospace", letterSpacing: "1px", textTransform: "uppercase" }}>Reality Score</div>
              </div>
            </div>
          </div>
          ...
          <div className="pp-rank-grid">
            {[
              ...
              <div key={i} className="pp-rank-card">
                ...
                <div style={{ fontSize: "42px", fontWeight: 900, color: "#fff", lineHeight: 1, letterSpacing: "-1px" }}>#{item.rank || "—"}</div>
          ...
          <div className="pp-stats-grid">
            {[
              ...
              <div key={i} className="pp-stat">
                ...
                {(item as any).sub && (
                  <div style={{ fontSize: "11px", color: "#2e2e2e", marginTop: "6px", fontFamily: "JetBrains Mono, monospace" }}>{(item as any).sub}</div>
                )}
              </div>
            ))}
          </div>
